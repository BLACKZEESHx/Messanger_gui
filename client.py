...
# https://www.youtube.com/shorts/NlD6udREoug
# ask for username
username = input('Enter a User Name >>> ')
# Create a new object of client that connect with server
client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
# Now connect to server
client.connect(("192.168.6.110", 14438))
# Main Client message receiver function

checking_if_server_asking_our_username = client.recv(1024).decode("utf-8")
if (checking_if_server_asking_our_username == "username?"):
    client.send(username.encode("utf-8"))
for i in range(2):
    greet_message = client.recv(1024).decode("utf-8")
    print("FromServer " + greet_message)

app = Ursina()
player = FirstPersonController(model="cube")
secplayer = Entity(model="cube", color=color.red)
ground = Entity(model="cube",collider="mesh", scale=20)
app.run(info=False)
...
# Main function that sends messages


def client_send():
    # Endless loop
    while True:
        input_of_message = input("")
        message = f'{username}: {input_of_message}\n'
        client.send(message.encode('utf-8'))


# Threading functions for receiving
receive_thread = threading.Thread(target=client_receive)
# Start receiving
receive_thread.start()

# Threading functions for sending messages
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # self.bar = Ui_Form2()
        # self.bar.setupUi(self)
        # self.bar.verticalLayout.addWidget(MyBar(self.bar))
        # MyBar(self.bar)
...
""")
    ...
    def show_send_message(self):
        ...
        client.send(messagegui.encode("utf-8"))
